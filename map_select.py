...
class Map_select:
    ...
    def Render_map_select(self) -> None:
        ...
        for key, pos in map_pos.items():
            px_pos: tuple[int, int] = (self.data.screen_size[0] // 2 - 12*map_zoom + pos[0] * 28*map_zoom, 20*map_zoom + pos[1] * 21*map_zoom)
            ...
            if pg.mouse.get_pos()[0] > px_pos[0] and pg.mouse.get_pos()[0] < px_pos[0] + self.map_images[key].get_width() and pg.mouse.get_pos()[1] > px_pos[1] and pg.mouse.get_pos()[1] < px_pos[1] + self.map_images[key].get_height():
                ...
                self.data.Draw_text(f"Difficulty: {map_difficulty[key].capitalize()}", 8 * self.data.hud_zoom, (220,220,40), (self.data.screen_size[0]//2 - 45*self.data.hud_zoom, 28*self.data.hud_zoom))
